...
def manual_blur(image, sigma=20):
    """Apply a Gaussian blur to an image manually using scipy"""
...
def create_blurred_background(video_clip, target_size=(1080, 1920)):
    """Create a blurred background from the first frame of the video using manual blur"""
    try:
        # Get the first frame
        frame = video_clip.get_frame(0)
        
        # Apply manual blur
        blurred_frame = manual_blur(frame, sigma=15)
        
        # Create an image clip from the blurred frame
        img_clip = ImageClip(blurred_frame)
        
        # Calculate size to maintain aspect ratio and ensure it covers the entire background
        ...
        # Resize and crop to fit target size
        bg_clip = (img_clip
                  .resize(width=new_width, height=new_height)
                  .crop(x_center=new_width/2, 
...
def create_final_video(video_path, transcript, output_path, font_file, style_key="style1"):
    """Create final video with TikTok-style subtitles and blurred background"""
    try:
        # Load the original video
        original_video = VideoFileClip(video_path, audio=True)
        
        # Define the target size for 9:16 ratio
        target_size = (1080, 1920)  # 9:16 aspect ratio for mobile
        
        # Create a blurred background using a frame from the video
        blurred_bg = create_blurred_background(original_video, target_size)
        if not blurred_bg:
            print("Failed to create blurred background. Falling back to default method.")
            # If we can't create a background, just return None and let the caller handle it
            original_video.close()
            return None
        
        # Calculate the size for the main video to maintain aspect ratio
        # We'll make it occupy about 80% of the screen width
        original_aspect = original_video.w / original_video.h
        
        # Scale video to fit in the 9:16 frame while maintaining aspect ratio
        if original_aspect > 9/16:  # Video is wider than target
            # Width is limiting factor
            new_width = int(target_size[0] * 0.9)  # 90% of frame width
            new_height = int(new_width / original_aspect)
        else:  # Video is taller than target
            # Height is limiting factor
            new_height = int(target_size[1] * 0.7)  # 70% of frame height
            new_width = int(new_height * original_aspect)
        
        # Resize the video
        resized_video = original_video.resize(width=new_width, height=new_height)
        
        # Center the video on the background
        video_pos = ((target_size[0] - new_width) // 2, 
                     (target_size[1] - new_height) // 2)
        
        # Create the composite video base (background + video)
        resized_video = resized_video.set_position(video_pos)
        base_video = CompositeVideoClip([blurred_bg, resized_video], size=target_size)
        
        # Get the audio from the original video
        base_video = base_video.set_audio(original_video.audio)
        ...
        # Use the new target_size for subtitle calculations
        sentences = group_words_into_sentences(
            words,
            max_words=MAX_WORDS_PER_SENTENCE,
            frame_width=target_size[0],
            font_size=int(target_size[1] * SUBTITLE_STYLES[style_key]["font-size"]),
            font_file=font_file,
        )
        ...
        # Create subtitle clips adapted to the new frame size
        all_clips = []
        for sentence in sentences:
            sentence_clips = create_sentence_clips(sentence, target_size, font_file, style_key)
        ...
        final_video = CompositeVideoClip([base_video] + all_clips, size=target_size)
