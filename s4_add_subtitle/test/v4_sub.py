...
def create_text_clip(text, size, color):
    return TextClip(
        text,
        color=color,
...
def create_box_clip(word, frame_size, position, duration):
    ...
    temp_clip = create_text_clip(word.word, font_size, BASE_TEXT_COLOR)
...
def create_sentence_clips(sentence_words, frame_size):
    ...
    for word in sentence_words:
        clip = create_text_clip(word.word, font_size, BASE_TEXT_COLOR)
...
def process_video(video_path, output_path):
    ...
    words = generate_subtitles(audio_path, os.getenv("OPENAI_API_KEY"))
