...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create curated video with specified duration')
    parser.add_argument('--duration', type=int, choices=[30, 45, 60], default=45,
                       help='Target duration in seconds (30, 45, or 60)')
    
    args = parser.parse_args()
    target_duration = args.duration
    
    # Get output path based on duration
    output_video = get_output_path(target_duration)
    
    print(f"Starting video curation process for {target_duration} seconds output...")
    ...
    try:
        ...
        # Find best segments
        print(f"Analyzing content and finding best segments for {target_duration}s video...")
        best_segments = find_best_segments(words, target_duration=target_duration, min_segment_duration=5)
