...
class SpeakerDetector:
    ...
    def detect_speaker(self, face_landmarks: List[dict], frame_shape: Tuple[int, int, int]) -> Dict[int, float]:
        height, width = frame_shape[:2]
        current_movements = {}
        
        for face_idx, landmarks in face_landmarks.items():
            if not landmarks:
                continue
                
            # Focus on mouth landmarks
            mouth_landmarks = [landmarks[i] for i in range(0, 468) if i in range(0, 17)]  # Mouth landmark indices
            
            if face_idx in self.prev_landmarks:
            ...
            self.prev_landmarks = face_landmarks.copy()
...
class VideoProcessor:
    ...
    def _detect_face_segments(self, video_path: str) -> Dict[int, List[dict]]:
        ...
        while cap.isOpened():
            ...
            if face_results.multi_face_landmarks:
                face_landmarks = {idx: landmarks for idx, landmarks in enumerate(face_results.multi_face_landmarks)}
                speaking_scores = self.speaker_detector.detect_speaker(face_landmarks, frame.shape)
    ...
    def _create_conversation_video(self, face_videos: Dict[int, str], original_video: str):
        ...
        while True:
            ...
            # Determine speaking face based on audio analysis
            # This is a simplified version - you might want to use more sophisticated
            # audio analysis for better speaker detection
    ...
    def _determine_speaking_face(self, frames: Dict[int, np.ndarray]) -> int:
        # This is a placeholder for more sophisticated speaker detection
        # You might want to use audio analysis or more advanced visual cues
