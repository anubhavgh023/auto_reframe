...
class PodcastToShortsConverter:
    ...
    def detect_faces(self, frame):
        ...
        Returns:
            list: List of detected face locations with confidence
        """
    ...
    def crop_to_vertical(self, frame, face_rect):
        """
        Crop frame to 9:16 vertical format with a wider view
        ...
        # Compute crop height (full frame height)
        crop_height = self.height

        # Wider crop width - approximately 2x the default
        crop_width = int(self.height * target_aspect * 1.5)
    ...
    def process_video(self, min_face_duration=0.5):
        ...
        while True:
            ...
            if faces:
                # If multiple faces, choose the largest with highest confidence
                largest_face = max(faces, key=lambda f: (f[2] * f[3], f[4]))
