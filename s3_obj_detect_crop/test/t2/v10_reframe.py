...
class FaceTracker:
    def __init__(self):
        ...
        self.face_crop_sizes = {}  # Store fixed crop sizes for each face
    ...
    def _get_crop_box(self, bbox, frame_shape):
        """Calculate crop box with fixed dimensions for each speaker"""
        ...
        # Use stored crop size if available for current face
        if self.current_face_id in self.face_crop_sizes:
            crop_width, crop_height = self.face_crop_sizes[self.current_face_id]
        else:
            # Calculate initial crop size (slightly wider than face)
            face_width = int(bbox.width * original_width * 1.5)  # 1.5x wider than face
            face_height = int(
                bbox.height * original_height * 1.5
            )  # 1.5x taller than face

            # Ensure minimum size and aspect ratio
            crop_height = max(
                face_height, int(original_height * 0.5)
            )  # At least 50% of frame height
            crop_width = int(crop_height * target_aspect_ratio)

            # Store the crop size for this face
            if self.current_face_id is not None:
                self.face_crop_sizes[self.current_face_id] = (crop_width, crop_height)
    ...
    def update(self, bbox, frame_shape):
        ...
        # Only switch face ID if we've seen the new face for enough frames
        if self.current_face_id != current_face_id:
            self.current_face_id = current_face_id
            self.position_buffer.clear()  # Clear position buffer on speaker switch
...
# Rest of the code remains the same (detect_faces and reframe_video_with_audio functions)
