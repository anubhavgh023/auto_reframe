...
class PodcastToShortsConverter:
    def __init__(self, input_video_path, output_dir):
        ...
        self.position_history = deque(maxlen=int(self.fps))  # 1 second buffer
        self.face_position_history = {}  # Dictionary to store face position history
        self.smoothing_window = int(self.fps * 0.5)  # Half second smoothing
    ...
    def smooth_face_position(self, face_id, position):
        """Apply temporal smoothing to face positions"""
        if face_id not in self.face_position_history:
            self.face_position_history[face_id] = deque(maxlen=self.smoothing_window)

        self.face_position_history[face_id].append(position)

        if len(self.face_position_history[face_id]) < 3:
            return position

        # Calculate weighted average of positions
        weights = np.linspace(0.5, 1.0, len(self.face_position_history[face_id]))
        weights = weights / np.sum(weights)

        smoothed_position = np.average(
            list(self.face_position_history[face_id]), weights=weights, axis=0
        )

        return tuple(map(int, smoothed_position))

    def get_stable_crop_window(self, frame, faces, speaker_idx):
        """Calculate stable crop window position"""
        frame_height, frame_width = frame.shape[:2]
        ...
        if not faces:
            target_x = default_pos
        else:
            speaker_face = None
            for face in faces:
                if face[4] == speaker_idx:
                    # Smooth face position
                    smoothed_face = self.smooth_face_position(
                        speaker_idx, (face[0], face[1], face[2], face[3])
                    )
                    speaker_face = (*smoothed_face, face[4])
                    break
            ...
            if speaker_face is None:
                ...
                target_x = max(
                    padding, min(target_x, frame_width - crop_width - padding)
                )

        # Apply temporal smoothing to crop window position
        self.position_history.append(target_x)
        smoothed_x = int(np.mean(self.position_history))

        # Add movement dampening
        if len(self.position_history) > 1:
            prev_x = self.position_history[-2]
            max_movement = int(crop_width * 0.01)  # Limit movement to 1% of crop width
            smoothed_x = prev_x + np.clip(
                smoothed_x - prev_x, -max_movement, max_movement
            )

        return max(0, min(smoothed_x, frame_width - crop_width))

    def crop_to_speaker(self, frame, faces, speaker_idx):
        """Crop frame to focus on the active speaker with stabilization"""
        crop_width = int(self.frame_height * 9 / 16)
        start_x = self.get_stable_crop_window(frame, faces, speaker_idx)
    ...
    def process_video(self):
        ...
        min_switch_frames = int(self.fps * 1.0)  # Increased to 1 second minimum
        ...
        while True:
            ...
            # More conservative speaker switching
