...
output_video_path = 'downloads/output_video.mp4'  # Final output video with audio included
...
def reframe_video_with_audio(input_path, output_path):
    ...
    temp_output_path = 'videos/temp_output_video.mp4'
    ...
    ffmpeg_command = [
        'ffmpeg',
        '-i', temp_output_path,   # Input processed video
        '-i', input_path,          # Input original video for audio
        '-c:v', 'copy',            # Copy the video stream
        '-c:a', 'aac',             # Set audio codec
        '-map', '0:v:0',          # Use video stream from processed video
        '-map', '1:a:0',          # Use audio stream from original video
